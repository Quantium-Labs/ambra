...
unsafe fn load_and_set_playback_artwork(url: String, for_counter: usize) {
    ...
    // Artwork is optional. Failed loads must never reach Objective-C artwork creation.
    if image == nil || !size.width.is_finite() || !size.height.is_finite()
...
unsafe fn load_image_from_url(url: &str) -> (id, CGSize) {
    ...
    // NSImage returns nil for missing files, failed downloads, and invalid images.
    // Sending `size` to it can trigger Rust's non-unwinding null-pointer panic.
    if image == nil {
        return (nil, CGSize::new(0.0, 0.0));
    }
    let size: CGSize = msg_send!(image, size);
...
mod artwork_tests {
    ...
    #[test]
    fn missing_artwork_does_not_abort_or_publish() {
        unsafe {
            let source = "file:///ambra-missing-artwork-regression-test.png";
            let (image, size) = load_image_from_url(source);
            assert_eq!(image, nil);
            assert_eq!(size.width, 0.0);
            assert_eq!(size.height, 0.0);
            load_and_set_playback_artwork(source.to_owned(), GLOBAL_METADATA_COUNTER.load(Ordering::SeqCst));
        }
    }

    #[test]
    fn non_image_file_is_rejected_safely() {
        unsafe {
            let (image, _) = load_image_from_url("file:///etc/hosts");
            assert_eq!(image, nil);
        }
    }

    #[test]
    fn valid_artwork_still_loads() {
        unsafe {
            let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../../../src/assets/images/fallbackCover.png").canonicalize().unwrap();
